...
build() {
	...
	echo "- repository"
	pushd "${COMMUNITY_PATH}" >/dev/null || exit
	$MVN_EXEC -q -ff -Dmaven.test.skip=true -Dskip.npm=true clean install || exit
	popd >/dev/null || exit
...
package-config() {
	...
	find . -type d | xargs -I {} mkdir -p "../../classes/org/{}"
	popd >/dev/null || exit
...
package-frontend() {
	...
	pushd "${COMMUNITY_PATH}/${SUBMODULE_PATH}" >/dev/null || exit
	find . -maxdepth 1 -type f -name '*.js'   | xargs -I {} cp {} "${EXTENSION_PATH}/${SUBMODULE_PATH}/target/debug/ng/{}"
	find . -maxdepth 1 -type f -name '*.json' | xargs -I {} cp {} "${EXTENSION_PATH}/${SUBMODULE_PATH}/target/debug/ng/{}"
	popd >/dev/null || exit
	cp -rf "${COMMUNITY_PATH}/${SUBMODULE_PATH}/src" "target/debug/ng/"
	cp -rf "src/main/ng" "target/debug/"
	pushd "target/debug" >/dev/null || exit
	find . -type d | xargs -I {} mkdir -p "../../src/generated/{}"
	popd >/dev/null || exit
